import {
  Phone,
  Mail,
  MapPin,
  FileCheck,
  Shield,
  Truck,
  Wrench,
  Car,
} from "lucide-react";
import { useLocation } from "react-router-dom";

export const Footer = () => {
  const location = useLocation();

  if (
    location.pathname !== "/" &&
    location.pathname !== "/politika-privatnosti"
  ) {
    return null;
  }

  return (
    <footer className="w-full bg-gray-900 relative overflow-hidden pt-16">
      <div className="absolute left-0 top-0 w-full h-full">
        <svg
          width="100%"
          height="100%"
          viewBox="0 0 1440 300"
          preserveAspectRatio="none"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M0 0H1440V300H0V0Z" fill="#1E2A3B" />
          <path
            d="M-300 100C150 150 450 300 1800 200"
            stroke="#293B62"
            strokeWidth="300"
            strokeLinecap="round"
          />
        </svg>
      </div>

      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          <div>
            <h3 className="text-white text-xl font-medium mb-6">O Nama</h3>
            <div className="mb-6">
              <img
                src="/logo as.svg"
                alt="Osiguranje Logo"
                className="h-12 mb-4"
              />
            </div>
            <p className="text-gray-300 mb-4">
              Specijalizovani smo za pružanje osiguranja i pomoći pri naplati
              odštete. Naš tim stručnjaka je tu da vam pomogne da ostvarite
              svoja prava na naknadu štete.
            </p>
            <div className="flex space-x-4 mt-4">
              <a
                href="#"
                className="w-10 h-10 bg-blue-800 rounded-full flex items-center justify-center hover:bg-teal-300 transition duration-300"
              >
                <svg
                  className="w-5 h-5 text-white"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path
                    fillRule="evenodd"
                    d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"
                    clipRule="evenodd"
                  ></path>
                </svg>
              </a>
              <a
                href="#"
                className="w-10 h-10 bg-blue-800 rounded-full flex items-center justify-center hover:bg-teal-300 transition duration-300"
              >
                <svg
                  className="w-5 h-5 text-white"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path
                    fillRule="evenodd"
                    d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z"
                    clipRule="evenodd"
                  ></path>
                </svg>
              </a>
              <a
                href="#"
                className="w-10 h-10 bg-blue-800 rounded-full flex items-center justify-center hover:bg-teal-300 transition duration-300"
              >
                <svg
                  className="w-5 h-5 text-white"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84"></path>
                </svg>
              </a>
            </div>
          </div>
          <div>
            <h3 className="text-white text-xl font-medium mb-6">
              Brzi Linkovi
            </h3>
            <ul className="space-y-3">
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <span className="mr-2">•</span> Početna
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <span className="mr-2">•</span> O Nama
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <span className="mr-2">•</span> Usluge
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <span className="mr-2">•</span> Naknada Štete
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <span className="mr-2">•</span> Blog
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <span className="mr-2">•</span> Kontakt
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <span className="mr-2">•</span> Politika Privatnosti
                </a>
              </li>
            </ul>
          </div>

          <div>
            <h3 className="text-white text-xl font-medium mb-6">Naše Usluge</h3>
            <ul className="space-y-3">
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <Shield className="w-4 h-4 mr-2 text-teal-300" /> Procena
                  Štete na Vozilima
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <Shield className="w-4 h-4 mr-2 text-teal-300" /> Nematerjalna
                  Šteta
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <Wrench className="w-4 h-4 mr-2 text-teal-300" /> Servis
                  Vozila
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300
                  transition duration-300 flex items-center"
                >
                  <Truck className="w-4 h-4 mr-2 text-teal-300" /> Transport
                  Vozila
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <FileCheck className="w-4 h-4 mr-2 text-teal-300" /> Naknada
                  Štete
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-gray-300 hover:text-teal-300 transition duration-300 flex items-center"
                >
                  <Car className="w-4 h-4 mr-2 text-teal-300" /> Rent-a-Car
                  Usluge
                </a>
              </li>
            </ul>
          </div>

          <div>
            <h3 className="text-white text-xl font-medium mb-6">
              Kontaktirajte Nas
            </h3>
            <ul className="space-y-4">
              <li className="flex items-start">
                <MapPin className="w-5 h-5 mr-3 text-teal-300 flex-shrink-0 mt-0.5" />
                <span className="text-gray-300">
                  Bulevar Kralja Aleksandra 12,
                  <br />
                  11000 Beograd, Srbija
                </span>
              </li>
              <li className="flex items-center">
                <Phone className="w-5 h-5 mr-3 text-teal-300 flex-shrink-0" />
                <a
                  href="tel:+38160318920"
                  className="text-gray-300 hover:text-teal-300 transition duration-300"
                >
                  +381 60 318 9200
                </a>
              </li>
              <li className="flex items-center">
                <Mail className="w-5 h-5 mr-3 text-teal-300 flex-shrink-0" />
                <a
                  href="mailto:info@osiguranje.rs"
                  className="text-gray-300 hover:text-teal-300 transition duration-300"
                >
                  info@osiguranje.rs
                </a>
              </li>
            </ul>
            <div className="mt-6">
              <a
                href="#"
                className="inline-block py-2 px-6 bg-teal-300 hover:bg-teal-400 text-gray-900 font-medium rounded-full transition duration-300 text-sm"
              >
                Besplatna Konsultacija
              </a>
            </div>
          </div>
        </div>

        <div className="border-t border-blue-800 py-8 mt-12">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-gray-400 text-sm mb-4 md:mb-0">
              &copy; {new Date().getFullYear()} Osiguranje. Sva prava zadržana.
            </p>
          </div>
        </div>
      </div>
    </footer>
  );
};
